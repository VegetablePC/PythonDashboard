...
st.markdown("<p style='margin-left: 15px; '>These two graphs display a difference in loss statistics between periods of storm activity and clear weather. As we can see, the amount of loss in dollars is clearly higher during periods of storm activity vs when clear weather occurs</p>", unsafe_allow_html=True)
